...
- "sources" : 이 아이디어를 도출할 때 "실제 근거로 삼은" 소스의 ID만 배열로 나열한다. 위 [Market Insight]·[2030 미래 트렌드]·[업로드 보고서] 블록에서 각 항목 앞에 붙은 [M#]·[T#]·[R#] 형식 ID를 대괄호 없이 그대로 쓴다(예: ["M3","T5","R2"]). 실제로 근거가 된 소스만 넣고, 내용과 무관한 소스는 절대 넣지 마라. 근거가 없으면 빈 배열 []. 자신의 아이디어 내용과 직접 연결되는 소스만 신중히 고른다.
