...
const SOURCE_GUIDE = {
  ...
  ci:      "경쟁사 전략 추적 보드(ci.samsungda.net)의 전략 프레임·실행축·최신 실행 증거",
...
async function handleGenerate(request, env) {
  ...
  // 경쟁사 전략 추적(CI) 실데이터 주입 (competitor_intelligence: strategies.json + evidence.json)
...
- "sources" : 이 아이디어를 도출할 때 "실제 근거로 삼은" 소스의 ID만 배열로 나열한다. 위 [Market Insight]·[2030 미래 트렌드]·[경쟁사 전략 추적]·[업로드 보고서] 블록에서 각 항목 앞에 붙은 [M#]·[T#]·[C#]·[R#] 형식 ID를 대괄호 없이 그대로 쓴다(예: ["M3","T5","C2","R2"]). 실제로 근거가 된 소스만 넣고, 내용과 무관한 소스는 절대 넣지 마라. 근거가 없으면 빈 배열 []. 자신의 아이디어 내용과 직접 연결되는 소스만 신중히 고른다.
예(명사형 종결): "content": ["월풀 CEO, 소비지출 <b>2008년 위기 수준</b> 경고", "북미 가전 수요 구조적 저점 진입"], "sources": ["M1","T5"]`;
...
${detailRule}`;
  ...
  return json({ ideas: ideas.slice(0, count), reportsUsed: useReports && !!reportBlock, reportSources, searchSources, miSources, trendSources, ciSources });
}
...
// ===== 경쟁사 전략 추적(CI) 실데이터 (competitor_intelligence 보드) =====
// 전략 프레임·실행축(strategies.json)과 최신 실행 증거(evidence.json)를 근거로 주입한다.
// ci.samsungda.net은 Worker+SSO 게이트라 서버-서버 fetch가 막힐 수 있어,
// 공개 GitHub raw CDN(원본 데이터)에서 직접 읽는다(changelog와 동일한 GitHub 소스 패턴).
const CI_RAW_BASE = "https://raw.githubusercontent.com/SimpleorNothing/competitor_intelligence/main/public/data";

async function gatherCI(topics, keyword, opts = {}) {
  const cap = opts.cap ?? 12;   // 증거 항목 상한
  const empty = { block: "", sources: [] };

  let strat = null, ev;
  try {
    const [sR, eR] = await Promise.all([
      fetch(`${CI_RAW_BASE}/strategies.json`, { cf: { cacheTtl: 600, cacheEverything: true } }),
      fetch(`${CI_RAW_BASE}/evidence.json`,   { cf: { cacheTtl: 600, cacheEverything: true } }),
    ]);
    if (!eR.ok) return empty;
    ev = await eR.json();
    if (sR.ok) { try { strat = await sR.json(); } catch (e) { strat = null; } }
  } catch (e) { return empty; }

  // 회사·축 이름 매핑 + 회사별 전략 프레임 (strategies.json)
  const compName = {}, axisName = {}, frameByComp = {};
  if (strat && Array.isArray(strat.companies)) {
    for (const c of strat.companies) {
      compName[c.id] = c.shortName || c.name || c.id;
      if (c.frame && c.frame.statement) frameByComp[c.id] = c.frame.statement;
      for (const a of (c.axes || [])) {
        axisName[a.id] = (a.code ? a.code + " " : "") + (a.title || "");
      }
    }
  }
  const axisLabel = (axisId) => {
    if (!axisId) return "";
    if (/-frame$/.test(axisId)) return "전략프레임";
    return axisName[axisId] || axisId;
  };

  const items = Array.isArray(ev.items) ? ev.items.filter(it => it && it.event) : [];
  if (!items.length) return empty;

  // 키워드 관련성: event·interpretation·회사·축 매칭 수
  const kwTokens = (keyword || "").trim()
    ? keyword.split(/[\s,·/]+/).filter(t => t.length >= 2)
    : [];
  const kwHit = (it) => {
    if (!kwTokens.length) return 0;
    const hay = [it.event, it.interpretation, compName[it.companyId], axisLabel(it.axisId)]
      .map(x => String(x || "")).join(" ");
    let n = 0;
    for (const tok of kwTokens) if (hay.includes(tok)) n++;
    return n;
  ...
  const sortFn = (a, b) => {
    const kw = kwHit(b) - kwHit(a);              // 키워드 관련성 최우선
    if (kw) return kw;
    return new Date(b.date || 0) - new Date(a.date || 0);  // 그다음 최신순
  };
  ...
  const top = arr.sort(sortFn).slice(0, cap).map((it, i) => ({ ...it, _id: "C" + (i + 1) }));

  const sources = top.map(it => {
    const comp = compName[it.companyId] || it.companyId || "";
    const ax = axisLabel(it.axisId);
    return {
      id: it._id,
      title: `${comp}${ax ? " · " + ax : ""} — ${String(it.event || "").slice(0, 70)}`.trim(),
      url: (it.source && it.source.url) || "",
      company: comp,
      axis: ax,
      date: it.date || "",
    };
  ...
  const lines = top.map(it => {
    const comp = compName[it.companyId] || it.companyId || "";
    const ax = axisLabel(it.axisId);
    const head = `[${it._id}] [${comp}${ax ? " · " + ax : ""}] ${it.date || ""}`;
    const ev1 = String(it.event || "").slice(0, 120);
    const interp = it.interpretation ? ` → ${String(it.interpretation).slice(0, 90)}` : "";
    return `- ${head} ${ev1}${interp}`;
  }).join("\n");

  const frames = Object.keys(frameByComp).length
    ? "\n[경쟁사 전략 프레임]\n" + Object.entries(frameByComp)
        .map(([cid, st]) => `· ${compName[cid] || cid}: ${String(st).slice(0, 90)}`).join("\n") + "\n"
    : "";

  const block = `[경쟁사 전략 추적 — CI 보드 실데이터]\n`
    + `· ci.samsungda.net 보드가 추적하는 경쟁사 전략 프레임·실행축과 최신 실행 증거다. 각 증거 앞 [C#]는 인용용 소스 ID다.\n`
    + (kwTokens.length ? `· 이번 키워드와 관련된 증거를 우선 정렬했다. 무관한 증거는 근거로 끌어오지 마라.\n` : ``)
    + `· 경쟁사 움직임에 대한 당사(삼성 DA)의 대응·차별화 관점에서 보고 주제를 도출하라.\n`
    + frames
    + `\n[최신 실행 증거]\n${lines}`;
  return { block, sources };
}
